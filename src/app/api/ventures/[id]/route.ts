...
export async function PATCH(
  ...
  const allowed = ['name', 'status', 'backlog_priority', 'design_partner_status', 'exploration_phase', 'one_metric_that_matters', 'notes', 'next_steps', 'primary_contact_id', 'timeline_visible', 'hidden_from_venture_tracker'];
  ...
  if (error && (error.message?.includes('notes') || error.message?.includes('next_steps') || error.message?.includes('does not exist'))) {
    ...
    const safeKeys = ['name', 'status', 'backlog_priority', 'design_partner_status', 'exploration_phase', 'one_metric_that_matters', 'primary_contact_id'];
