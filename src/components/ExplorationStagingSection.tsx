  ...
  return (
    <>
      <div
        ...
        {...attributes}
        {...listeners}
        className={`group flex min-w-[260px] max-w-[280px] shrink-0 cursor-grab flex-col rounded-xl border border-zinc-200 border-l-4 border-l-teal-400 bg-white p-4 shadow-sm ring-1 ring-zinc-900/5 transition-all hover:shadow-md active:cursor-grabbing ${isDragging ? 'opacity-50 ring-2 ring-teal-400' : ''}`}
      >
        ...
        {venture.tentative_start_date && (
          <div className="mt-1 text-xs text-zinc-500">
            <span className="font-medium">Tentative start:</span>{' '}
            {new Date(venture.tentative_start_date).toLocaleDateString()}
          </div>
        )}
        {venture.design_partner && (
          <div className="mt-1 text-xs text-zinc-500">
            <span className="font-medium">Design partner:</span> {venture.design_partner}
          </div>
        )}
      </div>
